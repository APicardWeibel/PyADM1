...
S_gas_h2 = initial_state["S_gas_h2"][
    0
]  # kg COD.m^-3 hydrogen concentration in gas phase
S_gas_ch4 = initial_state["S_gas_ch4"][
    0
]  # kg COD.m^-3 methane concentration in gas phase
S_gas_co2 = initial_state["S_gas_co2"][
    0
]  # kmole C.m^-3 carbon dioxide concentration in gas phas
...
# Precomputing s values for eqn10 (depend only on parameter values)
s_1 = (
    -1 * C_xc
    + f_sI_xc * C_sI
    + f_ch_xc * C_ch
...
gasflow = pd.DataFrame(index=range(len(t) + 1), columns=["q_gas", "q_ch4"])
gasflow.iloc[0] = [0, 0]
total_ch4 = 0
...
print("Elapsed time before main loop in seconds:", end_time - start_time)
start_time = process_time()

## Dynamic simulation
# Loop for simlating at each time step and feeding the results to the next time step
for u in t[1:]:
    state_input = influent_state.loc[
        n,
        [
            "S_su",  # kg COD.m^-3
            "S_aa",  # kg COD.m^-3
            ...
            "X_h2",  # kg COD.m^-3
            "X_I",  # kg COD.m^-3
            "S_cation",  # kmole N.m^-3
            "S_anion",  # kmole N.m^-3
            "Q",
        ],
    ]

    S_h2_in = state_input["S_h2"]  ## Used later on in DAE
    ...
    (
        ...
        S_gas_ch4,
        S_gas_co2,
    ) = simulate(t_step=t_step, solvermethod=solvermethod)
    ...
    simulate_results.iloc[n] = dfstate_zero
    t0 = u
